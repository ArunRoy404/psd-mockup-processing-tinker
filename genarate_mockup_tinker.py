...
class MockupApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PSD Layered Mockup Engine & Realtime Preview")
    ...
    def setup_ui(self):
        ...
        tk.Label(main_frame, text="PSD Layered Mockup Generator", font=("Arial", 20, "bold"), bg="#f3f4f6").pack(pady=(0, 10))

        # Top File Selection Controls
        ctrl_frame = tk.LabelFrame(main_frame, text=" 1. Select PSD File ", font=("Arial", 11, "bold"), bg="#f3f4f6", padx=10, pady=10)
        ...
        # Options Box
        opt_frame = tk.LabelFrame(main_frame, text=" 2. Global Options ", font=("Arial", 11, "bold"), bg="#f3f4f6", padx=10, pady=5)
        opt_frame.pack(fill="x", pady=5)

        tk.Label(opt_frame, text="Fit Mode:", bg="#f3f4f6").grid(row=0, column=0, padx=5, sticky="w")
        self.fit_var = tk.StringVar(value="fill")
        ttk.Combobox(opt_frame, textvariable=self.fit_var, width=12, state="readonly",
                     values=["fill", "fit", "stretch", "none"]).grid(row=0, column=1, padx=5, sticky="w")
        ...
        tk.Button(btn_frame, text="GENERATE MOCKUP PREVIEW", command=self.process, bg="#10b981", fg="white",
                  font=("Arial", 12, "bold"), height=2).pack(fill="x")

        # Canvas Preview
        self.preview_canvas = tk.Canvas(main_frame, width=800, height=420, bg="#d1d5db", highlightthickness=1)
